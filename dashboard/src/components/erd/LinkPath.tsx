...
import { commentHeight, fieldHeight, tableWidth, titleHeight } from "@/utils/erd/settings";
 ...
 */
export default function LinkPath(props: any) {
    const { link, setEditingLink } = props;
    const { tableDict, version } = useContext(GraphStateContext)
    ...
    const [sourceFieldIndex, targetFieldIndex] = [
        sourceTable.fields.findIndex((field: any) => field.id === endpoints[0].fieldId),
        targetTable.fields.findIndex((field: any) => field.id === endpoints[1].fieldId),
    ];
    ...
    if (endpoints[0].id == endpoints[1].id) {
        ...
        C ${sourceRight + control * factor} ${y} ${sourceRight + control * factor} ${y1} ${sourceRight + control
            } ${y1}
        L ${sourceRight} ${y1}`;
